...
// @Summary      UploadAttendance 
// @Description   This API performs the UPLOAD operation on Attendance. It allows you to  Attendance records.
// @Tags          Attendance
// @Accept       json
// @Produce      json
...
// @Router      /UploadAttendance [UPLOAD]

    func UploadAttendanceApi(c *fiber.Ctx) error {
	...
	for {
		...
		var AttendanceStruct dto.Attendance
		if err := json.Unmarshal(objectJSON, &AttendanceStruct); err != nil {
			fmt.Println("Error unmarshalling object JSON:", err)
			return utils.SendErrorResponse(c, fiber.StatusInternalServerError, err.Error())
		}

		AttendanceStruct.AttendanceId = uuid.New().String()


		err = dao.DB_CreateAttendance(&AttendanceStruct)
		if err != nil {
			return utils.SendErrorResponse(c, fiber.StatusInternalServerError, err.Error())
		}
	}
