...
func DB_FindallAttendance(page, size, searchTerm string, noPagination bool) (int64, *[]dto.Attendance, error) {
	var skip int64 = 0
	var limit int64 = 0
	...
	if searchTerm != "" {
		searchConditions := []bson.M{
			{"attendanceid": bson.M{"$regex": searchTerm, "$options": "i"}},
	...
	cursor, err := dbConfig.DATABASE.Collection("Attendances").Aggregate(ctx, pipeline)
	if err != nil {
		return 0, nil, err
	}
	...
	var objects []dto.Attendance
	if len(results.Data) > 0 {
		objects = results.Data
	} else {
		objects = []dto.Attendance{}
	}
