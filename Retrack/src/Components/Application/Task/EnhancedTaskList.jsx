...
import { Card, CardHeader, CardBody, Table, Input, Button, Row, Col } from 'reactstrap';
import { CreatedByMe, Print, TodayTasks, DelayedTasks, UpcomingTasks, ThisWeekTask, ThisMonthTasks, AssignedToMe, MyTasks } from '../../../Constant';
...
const EnhancedTaskList = ({ activeFilter = 'all', tagFilter = '' }) => {
    ...
    const applyFilters = () => {
        ...
        switch (activeFilter) {
            case 'createdByMe':
                filtered = filtered.filter(task => task.assigned_by_email === user?.email);
                break;
            case 'today':
                const today = new Date().toISOString().split('T')[0];
                filtered = filtered.filter(task =>
                    task.due_date && task.due_date.split('T')[0] === today
                );
                break;
            case 'delayed':
                filtered = filtered.filter(task => {
            ...
            case 'thisMonth':
                ...
                break;
            case 'assignedToMe':
                filtered = filtered.filter(task => task.assigned_to_email === user?.email);
                break;
            case 'myTasks':
                filtered = filtered.filter(task => task.assigned_to_email === user?.email);
                break;
    ...
    const getFilterTitle = () => {
        switch (activeFilter) {
            case 'createdByMe': return CreatedByMe;
            case 'today': return TodayTasks;
            case 'delayed': return DelayedTasks;
            case 'upcoming': return UpcomingTasks;
            case 'thisWeek': return ThisWeekTask;
            case 'thisMonth': return ThisMonthTasks;
            case 'assignedToMe': return AssignedToMe;
            ...
            <Card className="mb-0">
                <CardHeader className="d-flex justify-content-between align-items-center">
                    ...
                    <div className="d-flex align-items-center">
                        ...
                        <ReactToPrint
                            trigger={() => (
                                <a href="#javascript" className="ms-2"><Printer className="me-1" />{Print}</a>
                            )}
                                    ...
                                    {filteredTasks && filteredTasks.length ?
                                        filteredTasks.map((task) => {
                                            return (
                                                <tr key={task.task_id}>
                                                    <td>
                                                        <H6 attrH6={{ className: 'task_title_0' }} >{task.title}</H6>
                                                    </td>
                                                    <td>
                                                        <P attrPara={{ className: 'task_desc_0' }} >{task.description}</P>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <span className={`badge ${
                                                            ...
                                                            task.status === 'In Progress' ? 'badge-warning' : 'badge-primary'
                                                        }`}>
                                                            {task.status}
                                                        </span>
                                                    </td>
                                        ...
                                        : <tr><td colSpan="6"><div className="no-favourite text-center p-3"><span>No tasks found</span></div></td></tr>
                                    }
                                </tbody>
                            </Table>
                        </div>
